...
asbq_psqi_tbl <- reactive({
  ...
  # asbq_psqi_df %>%
  #   dplyr::filter(date_administered %in% ymd("2021-11-25"))
...
output$table2 <- DT::renderDataTable({
  ...
  asbq_psqi_summary <- asbq_psqi_tbl() %>%
    ...
    mutate(
      ...
      Gender = `gender`,
      `ASBQ Score` = asbq_global,
      `Sleep efficiency (%)` = round(percentage,2),
      `Sleep duration (mins)` = combined_mins,
      `Sleep duration (hrs)` = round(combined_mins/60,2),
      `PSQI Score` = `psqi_total`
      ) %>% 
    select(8:15)
...
output$table3 <- DT::renderDataTable({
  ...
  asbq_psqi_summary2 <- asbq_psqi_tbl() %>%
    ...
    select(5:9) %>% 
    summarise(
      `Average ASBQ Score` = round(mean(`ASBQ Score`),2),
      `Average PSQI Score` = round(mean(`PSQI Score`),2),
      `Average sleep duration (mins)` = round(mean(`Sleep duration (mins)`),2),
      `Average sleep duration (hrs)` = round(mean(`Sleep duration (hrs)`),2),
      `Average sleep efficiency (%)` = round(mean(`Sleep efficiency (%)`),2)
    )
